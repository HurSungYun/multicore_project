...
void conv2d_gpu(Tensor input, Tensor filter, Tensor bias, Tensor &output, int idx, int step, bool init_flag, cl_mem &filter_d, cl_mem &bias_d) {
  ...
  Tensor new_input;
  new_input.alloc_once({OH, OW, R, S, C});

  for (size_t oh = 0; oh < OW; oh++) {
        ...
        for (size_t s = 0; s < S; s++) {
          for (size_t c = 0; c < C; c++) {
            ...
            if (ih < 0 || ih >= H || iw < 0 || iw >= W) {
              new_input.buf[oh * OW * R * S * C + ow * R * S * C + r * S * C + s * C + c] = 0;
            } else {
              new_input.buf[oh * OW * R * S * C + ow * R * S * C + r * S * C + s * C + c] = input.buf[ih * W * C + iw * C + c];
            }
          }
        }
      }
    ...
    for (size_t oh = 0; oh < OW; oh++) {
      for (size_t ow = 0; ow < OW; ow++) {
        ...
        for (size_t r = 0; r < R; r++) {
          for (size_t s = 0; s < S; s++) {
            for (size_t c = 0; c < C; c++) {
              float ii = new_input.buf[oh * OW * R * S * C + ow * R * S * C + r * S * C + s * C + c];
...
*/
  ...
  cl_mem input_d = clCreateBuffer(context, CL_MEM_READ_WRITE, OH * OW * R * S * C * sizeof(float), NULL, &err);
  CHECK_ERROR(err);
  if (init_flag) {
...
//  err = clEnqueueWriteBuffer(data_queue, input_d, CL_TRUE, 0, H * W * C * sizeof(float), input.buf, 0, NULL, &data_wait);
  err = clEnqueueWriteBuffer(data_queue, input_d, CL_TRUE, 0, OH * OW * R * S * C * sizeof(float), new_input.buf, 0, NULL, &data_wait);
  CHECK_ERROR(err);
